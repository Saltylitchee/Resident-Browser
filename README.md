...
### 1. 3段階の表示ステート制御
作業の集中度や必要性に応じて、以下の状態を切り替え可能です。

- **小窓モード (Float)**: 最前面固定。学習・開発時のメインリファレンス用。
- **インジケーターモード (Indicator)**: 画面端に最小化。情報を保持しつつ、即座に復元可能。
- **完全非表示 (Hidden)**: 視界から完全に消去し、メイン作業に没頭。

![小窓モードとエディタを並べた様子]
*キャプション：エディタの横にリファレンスを配置し、視線移動を最小化している様子*

### 2. メディア・クイックコントロール
インジケーター状態では、特定のページにおいてアイコンクリックのみで再生/停止を操作可能。作業のリズムを維持したまま音声を制御できます。

![インジケーターモードの表示]
*キャプション：画面端に常駐するインジケーター。クリックで直感的な操作が可能*
